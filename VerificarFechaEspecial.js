async function verificarFechaEspecial() {
  try {
    ...
    // 5. Tomodr Id del elemento al que se le va a colocar el mensaje
    const valor = document.getElementById("valor");
    const fondo = document.getElementById("fondo");
    /*
    // 6. Primero verifica por número de día del año (ej. 256)
    if (datos.dias_del_ano[diaDelAno]) {
      mensaje = datos.dias_del_ano[diaDelAno];
    } 
    // 7. Luego verifica por fecha específica (ej. 2026-09-11)
    else if (datos.fechas_especificas[fechaFormateada]) {
    ...
    */
    ...
    if (evento) {
      ...
      console.log(`%c${evento.mensaje}`, `color: ${evento.color}; font-weight: bold; font-size: 16px;`);
      return evento;
    } else {
